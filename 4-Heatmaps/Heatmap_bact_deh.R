relab_bact_deh_core <- transform_sample_counts(illumina_dehesa_final, function(x) x / sum(x))
relab_bact_deh_core

# Merge by phylum filo
relab_bact_deh_core <-  tax_glom(relab_bact_deh_core, "Phylum")

# Add tree influence as variable
core_bact_deh_data <- data.frame(sample_data(relab_bact_deh_core))
core_bact_deh_data$Tree_inf <- core_bact_deh_data$Porcentaje_defol
core_bact_deh_data$Tree_inf <- ifelse(is.na(core_bact_deh_data$Tree_inf) == TRUE, 150, core_bact_deh_data$Tree_inf)
colnames(core_bact_deh_data)[c(15,30,31, 36, 43,46,45,50,49,53)] <- c("Defol", "Temp", "Prec","Org_C", "Av_P", "NH4_N", "NO3_N", "Ramm", "Rnit", "RH")
sample_data(relab_bact_deh_core) <- core_bact_deh_data


# MicrobiomeSEQ -----
taxa_names(relab_bact_deh_core) <- paste("OTU_ID", seq(1,length(as.vector(tax_table(relab_bact_deh_core)[,2]))), sep = "_")
colnames(core_bact_deh_data)
correlacion_nut_func <- taxa.env.correlation(relab_bact_deh_core, grouping_column = "Land-use", method = "pearson", 
...
# Add taxonomy to df
relab_bact_deh_core_tax_table <- tax_table(relab_bact_deh_core)
correlacion_nut_func$Phylum <- as.factor(relab_bact_deh_core_tax_table[match(correlacion_nut_func$Taxa, rownames(relab_bact_deh_core_tax_table)), 2])
