relab_fungi_opw_core <- transform_sample_counts(mycota_openwood_final, function(x) x / sum(x))
relab_fungi_opw_core

# Merge by phylum filo
relab_fungi_opw_core <-  tax_glom(relab_fungi_opw_core, "Phylum")

# Add tree influence as variable
core_fungi_opw_data <- data.frame(sample_data(relab_fungi_opw_core))
core_fungi_opw_data$Tree_inf <- core_fungi_opw_data$Porcentaje_defol
core_fungi_opw_data$Tree_inf <- ifelse(is.na(core_fungi_opw_data$Tree_inf) == TRUE, 150, core_fungi_opw_data$Tree_inf)
colnames(core_fungi_opw_data)[c(15,30,31, 36, 43,46,45,50,49,53)] <- c("Defol", "Temp", "Prec","Org_C", "Av_P", "NH4_N", "NO3_N", "Ramm", "Rnit", "RH")
sample_data(relab_fungi_opw_core) <- core_fungi_opw_data


# MicrobiomeSEQ -----
taxa_names(relab_fungi_opw_core) <- paste("OTU_ID", seq(1,length(as.vector(tax_table(relab_fungi_opw_core)[,2]))), sep = "_")
colnames(core_fungi_opw_data)
correlacion_nut_func <- taxa.env.correlation(relab_fungi_opw_core, grouping_column = "Manejo_Conteo", method = "pearson", 
...
# Add taxonomy to df
relab_fungi_opw_core_tax_table <- tax_table(relab_fungi_opw_core)
correlacion_nut_func$Phylum <- as.factor(relab_fungi_opw_core_tax_table[match(correlacion_nut_func$Taxa, rownames(relab_fungi_opw_core_tax_table)), 2])
...
correlacion_nut_func <- rbind(correlacion_nut_func, Entomophthoromycota, Glomeromycota, Olpidiomycota)
